...
static void init_diplomacy_menu(struct menu_diplo *menuDip) 
{
    menuDip->on = 0;
    initTextInfo(&menuDip->errorText, " ", littleFont, 80, 400, 700, (SDL_Color){255, 255, 255, 255}, 0, 0, 0);
    const char *diplo_options[] = {
        "Set Overlord", "Add ally", "Remove ally", "Add enemy", "Remove enemy"
    };
    menuDip->diploSelect = malloc(sizeof(DropDown));
    initDropDown(menuDip->diploSelect, 300, 400, 5, 300, 80, allocate_and_initialize_options(diplo_options, 5),
                 (SDL_Color) {255, 255, 255, 255}, (SDL_Color) {169, 169, 169, 255}, (SDL_Color){0, 0, 0, 255}, littleFont);
    initTextBox(&menuDip->diploTextBox, 800, 400, 400, 90, (SDL_Color){0, 0, 0, 255}, (SDL_Color){255, 255, 255, 255}, (SDL_Color){0, 0, 0, 255}, littleFont, false);
    initTextBox(&menuDip->bgDiploTextBox, 200, 350, 1100, 300, (SDL_Color){165, 132, 86, 255}, (SDL_Color){165, 132, 86, 255}, (SDL_Color){165, 132, 86, 255}, littleFont, false);
}

static void init_inventory_menu(struct menu_inventaire *menuInv) 
{
    menuInv->on = 0;
    menuInv->enter = 0;
    const char *inventory_options[] = {"", "", "", "", "", "", "", "", "", ""};
    menuInv->selector = initializeSelector(100, 50, 400, 50, (SDL_Color){75, 0, 130, 255}, (SDL_Color){221, 160, 221, 255}, (SDL_Color){255, 255, 255, 255}, littleFont, allocate_and_initialize_options(inventory_options, 10), 10);
...
static void init_trade_menu(struct menu_trade *menuTrad) 
{
    menuTrad->on = 0;
    menuTrad->tab = -1;
    menuTrad->count1 = 1;
    ...
    menuTrad->selector2 = initializeSelector(550, 50, 400, 50, (SDL_Color){75, 0, 130, 255}, (SDL_Color){221, 160, 221, 255}, (SDL_Color){255, 255, 255, 255}, littleFont, allocate_and_initialize_options(trade_options, 10), 10);
}
...
static void init_condition_menu(struct cond_menu *menu_cond) 
{
    ...
    // Initialize formation space and layout
    ...
    // Initialize buttons for managing formation
    ...
    // Initialize trade decision selector
...
void init_main_menu(void) 
{
    ...
    // Initialize submenus
    init_diplomacy_menu(main_menu->menuDip);
