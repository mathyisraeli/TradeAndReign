...
int find_extreme_altitude_house(struct personnages *p, int coo, int *lowgo, int *highgo)
{
    p = p;
    int lowest = ground_altitude[coo];
    int highest = ground_altitude[coo];
    *lowgo = coo;
    *highgo =  coo;
    for (int i = 1; i < house2[0][0]+1; i++)
    {
        ...
        {
            if (lowest >= ground_altitude[coo+(i-1)*max_x + j])
            {
                *lowgo = coo+(i-1)*max_x + j;
                lowest = ground_altitude[coo+(i-1)*max_x + j];
            }
            else if (ground_altitude[coo+(i-1)*max_x + j] >= highest)
            {
                *highgo = coo+(i-1)*max_x + j;
                highest = ground_altitude[coo+(i-1)*max_x + j];
            }
        }
    }
    if (highest == lowest + 1)
    ...
    else 
    {
        ...
        else if (p->house_id < 0) // i dont have a house
        {
            ...
            else // i should build a house
            {
                ...
                {
                    if (check_if_surface_is_flat(p, -p->house_id) == 0)
                    {
                        int lowgo; int highgo;
                        int whereiam = moix + moiy * max_x;
                        int check = find_extreme_altitude_house(p, -p->house_id, &lowgo, &highgo);
                        if (check != 0)
                        {
                            if (go_to_top(p, check) == 1)
                            {
                                SDL_Texture *t = ground_texture[(int)p->x + ((int)p->y)*max_x];
                                if (t == img->t->he1 || t == img->t->he2 || t == img->t->he3 || t == img->t->he4 || t == img->t->he5)
                                    sprintf (ordre + strlen(ordre), "%d 16 +1 herbe %d 08 %d %d 22 2 %d 21 0 ", p->id, p->id, (int)p->x + ((int)p->y)*max_x, p->id, p->id);
                                else if (t == img->t->sa1 || t == img->t->sa2 || t == img->t->sa3)
                                    sprintf (ordre + strlen(ordre), "%d 16 +1 sable %d 08 %d %d 22 2 %d 21 0 ", p->id, p->id, (int)p->x + ((int)p->y)*max_x, p->id, p->id);
                                else if (t == img->t->ne1 || t == img->t->ne2 || t == img->t->ne3)
                                    sprintf (ordre + strlen(ordre), "%d 08 %d %d 13 %d ea1 ", p->id, (int)p->x + ((int)p->y)*max_x, p->id, (int)p->x + ((int)p->y)*max_x); 
                            }
                        }
                        if (whereiam == lowgo && have_ground_in_inventory(p->i_list) == 1)
                        {
                            put_ground(p);
                        }
                        else if (whereiam != lowgo && 10 > n_item(p->i_list))
                        {
                            SDL_Texture *t = ground_texture[(int)p->x + ((int)p->y)*max_x];
                            if (t == img->t->he1 || t == img->t->he2 || t == img->t->he3 || t == img->t->he4 || t == img->t->he5)
                                sprintf (ordre + strlen(ordre), "%d 16 +1 herbe %d 08 %d %d 22 2 %d 21 0 ", p->id, p->id, (int)p->x + ((int)p->y)*max_x, p->id, p->id);
                        ...
                        else if (have_ground_in_inventory(p->i_list) == 1)
                        {
                            int togox = lowgo%max_x;
                            int togoy = lowgo/max_x;
                            sprintf(ordre + strlen(ordre), "%d 03 %f %d 04 %f ", p->id, togox+0.5, p->id, togoy+0.5);
                        }
                        else
                        {
                            int togox = highgo%max_x;
                            int togoy = highgo/max_x;
                            sprintf(ordre + strlen(ordre), "%d 03 %f %d 04 %f ", p->id, togox+0.5, p->id, togoy+0.5);
                        }
                    }
                    else
